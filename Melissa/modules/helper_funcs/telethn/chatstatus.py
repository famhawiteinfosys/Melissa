...
from Melissa import DRAGONS
from Melissa.modules.helper_funcs.telethn import IMMUNE_USERS, telethn
...
async def Melissa_is_admin(chat_id: int):
    status = False
    Melissa = await telethn.get_me()
    async for user in telethn.iter_participants(
        chat_id, filter=ChannelParticipantsAdmins
    ):
        if Melissa.id == user.id:
            status = True
            break
    return status
